    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_TARGET
            {
                ...
                float uv = blockCenter;
                float2 boxSize = fwidth(uv) * _MainTex_TexelSize.zw;
                float tx = uv * _MainTex_TexelSize.zw;
                float2 txOffset = clamp(frac(tx)/boxSize, 0, 0.5) - clamp((1 - frac(tx))/boxSize, 0, 0.5);

                float2 final_uv = (floor(tx) + 0.5 + txOffset) * _MainTex_TexelSize.xy;

                //tex = SAMPLE_TEXTURE2D(_MainTex, sampler_bilinear_clamp, final_uv);
                return tex;
            }
            ENDHLSL
        }
